...
/* ═══════════════════════════════════════════════════════════════════════════
   RedasP — server.js  v3.5  (headers exatos da requisição real)
   Baseado na captura: POST /registration/edusp/token com headers mínimos
═══════════════════════════════════════════════════════════════════════════ */
...
const PORT          = (CFG.server?.port || 3000) + 1;
...
/* ═══════════════════════════════════════════════════════════════════════════
   HEADERS (replicando exatamente a captura)
═══════════════════════════════════════════════════════════════════════════ */
...
// Headers EXATOS da captura para a segunda etapa
function buildEduspTokenHeaders() {
  return {
    'Content-Type': 'application/json',
    'Accept': 'application/json',
    'x-api-realm': 'edusp',
    'x-api-platform': 'webclient',
    // Headers adicionais para evitar bloqueio
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36',
    'Origin': 'https://saladofuturo.educacao.sp.gov.br',
    'Referer': 'https://saladofuturo.educacao.sp.gov.br/',
...
// Headers para as demais chamadas (usando auth_token)
...
async function sedLogin(ra, senha) {
  ...
  // 2ª etapa: Obter auth_token da EDUSP (com headers exatos da captura)
  const tokenHeaders = buildEduspTokenHeaders(); // sem captcha token
  ...
  log.ok('AUTH', `EDUSP auth_token obtido — nick: ${nick}`);
  return { auth, nick, sedTok };
}

/* ═══════════════════════════════════════════════════════════════════════════
   SESSIONS E ROTAS (mantenha o que você já tem, apenas com as funções de IA)
═══════════════════════════════════════════════════════════════════════════ */
// ─── SESSION STORE ──────────────────────────────────────────────────────
...
async function renewAuth(sess) {
  log.info('AUTH', `Relogando ${sess.ra}...`);
  try {
    ...
    log.error('AUTH', 'Renovação falhou: '+e.message);
    return false;
  }
}
...
/* ─── FUNÇÕES DE IA E PROCESSAMENTO (copie do seu arquivo original) ── */
// ⚠️ ATENÇÃO: Você precisa copiar as funções:
// cleanHtml, getEssayContext, studentHeader, buildHalfPrompt, buildFullPrompt,
// normalizeEssayText, extractTitle, mergeHalves, pickBestParagraphs,
// generateEssayText, processOneEssay
// do seu arquivo original e colá-las aqui (estão no seu backup).
// Por questões de tamanho, não as reescrevi.
...
app.listen(PORT, ()=>{
  ...
  console.log(`\x1b[36m  ╚═╝  ╚═╝╚══════╝╚═════╝ ╚═╝  ╚═╝╚══════╝╚═╝     \x1b[0m`);
  console.log('');
  log.ok('SYS', `RedasP v3.5 → http://localhost:${PORT}`);
