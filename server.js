...
/* ═══════════════════════════════════════════════════════════════════════════
   RedasP — server.js  v3.2  (usa token SED como x-api-key)
...
const EDUSP         = 'https://edusp-api.ip.tv'; // domínio correto
...
/* ═══════════════════════════════════════════════════════════════════════════
   HEADERS PARA SED E EDUSP (usando token SED como x-api-key)
═══════════════════════════════════════════════════════════════════════════ */
...
/* ─── LOGIN DIRETO (usa token SED como chave) ────────────────────────── */
async function sedLogin(ra, senha) {
  ...
  // Usamos o token SED como chave de API (x-api-key) para a EDUSP
  // Pula a segunda etapa (registration/edusp/token) que está bloqueada
  const nick = w.body?.DadosUsuario?.NM_NICK || w.body?.DadosUsuario?.NAME || ra;
  log.ok('AUTH', `Token SED configurado como x-api-key — nick: ${nick}`);
  return { auth: sedTok, nick, sedTok };
}
...
app.listen(PORT, ()=>{
  ...
  console.log(`\x1b[36m  ╚═╝  ╚═╝╚══════╝╚═════╝ ╚═╝  ╚═╝╚══════╝╚═╝     \x1b[0m`);
  console.log('');
  log.ok('SYS', `RedasP v3.2 → http://localhost:${PORT}`);
